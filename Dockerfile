...
RUN mkdir -p $M2_HOME/org/talend/components/servicenow/0.0.1-SNAPSHOT $M2_HOME/org/talend/components/widget/1.0.0-SNAPSHOT && \
    ...
    for i in "" .asc; do wget "https://oss.sonatype.org/service/local/artifact/maven/content?r=snapshots&g=org.talend.components&a=widget&v=1.0.0-SNAPSHOT&e=jar$i" -O $M2_HOME/org/talend/components/widget/1.0.0-SNAPSHOT/widget-1.0.0-SNAPSHOT.jar$i; done && \
